...
void ModUpdateTask::executeTask()
{
    ...
    // The dialogs below run nested event loops, so the launch may have been aborted while they were open
    if (!isRunning()) {
        return;
    }
