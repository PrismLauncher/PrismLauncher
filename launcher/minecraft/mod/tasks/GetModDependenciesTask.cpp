...
Task::Ptr GetModDependenciesTask::getProjectInfoTask(const std::shared_ptr<PackDependency>& pDep)
{
    ...
    connect(info.get(), &Task::succeeded, this, [result, pDep] { *pDep->pack = **result; });
    return info;
}
                                                        ...
                                                        int level)
{
    ...
    auto [version, result] = getAPI(provider)->getDependencyVersion(args).make();
    if (!version) {
        return tasks;
    }
    connect(version.get(), &Task::succeeded, this, [dep, provider, pDep, level, this, result] {
        auto& pack = *result;
        pDep->version = pack;
        ...
        if (dep.addonId.toString().isEmpty() && !pDep->version.addonId.toString().isEmpty()) {
            ...
            auto dep2 = getOverride({ .addonId = pDep->version.addonId, .type = pDep->dependency.type, .version = "" }, provider);
            if (dep2.addonId != pDep->version.addonId) {
                removePack(pDep->version.addonId);
                addTask(prepareDependencyTask(dep2, provider, level));
            } else {
        ...
        for (const auto& dep2 : getDependenciesForVersion(pDep->version, provider)) {
            addTask(prepareDependencyTask(dep2, provider, level - 1));
        }
    });
    connect(version.get(), &Task::failed, this, [](const QString& reason) {
...
#else
    for (auto it = m_packDependencies.begin(); it != m_packDependencies.end();)
        if (pred(*it))
            it = m_packDependencies.erase(it);
        else
            ++it;
#endif
}
