...
typedef QMap<int, std::pair<Version, Version>> FormatMap;

// Values taken from:
// https://minecraft.fandom.com/wiki/Tutorials/Creating_a_resource_pack#Formatting_pack.mcmeta
static const FormatMap s_pack_format_versions = {
...
int ResourcePack::fuzzyFormat(Version version)
{
    for (FormatMap::const_iterator iterator = s_pack_format_versions.constBegin(); iterator != s_pack_format_versions.end(); iterator++)
        if (version >= iterator->first && version <= iterator->second)
            return iterator.key();

    return s_pack_format_versions.keys().last();
}

std::pair<Version, Version> ResourcePack::formatCompatibleVersions(int format)
{
    if (!s_pack_format_versions.contains(format)) {
        return { {}, {} };
    }

    return s_pack_format_versions.constFind(format).value();
}
...
{
    return formatCompatibleVersions(m_pack_format);
}
