...
void InstanceImportTask::processFlame()
{
    ...
    if (!m_extraInfo.isEmpty()) {
        ...
        instCreationTask =
            makeShared<FlameCreationTask>(m_stagingPath, m_trustedSource, m_parent, packId, packVersionId, originalInstanceId);
    } else {
        // FIXME: Find a way to get IDs in directly imported ZIPs
        instCreationTask = makeShared<FlameCreationTask>(m_stagingPath, m_trustedSource, m_parent, QString(), QString());
    }
...
void InstanceImportTask::processTechnic()
{
    ...
    packProcessor->run(name(), m_instIcon, m_stagingPath);
}
...
void InstanceImportTask::processMultiMC()
{
    ...
    if (!conf.has_value()) {
        emitFailed(tr("Could not load extracted instance.cfg: %1").arg(conf.error()));
        return;
    }
...
void InstanceImportTask::processModrinth()
{
    ...
    if (!m_extraInfo.isEmpty()) {
        ...
        instCreationTask =
            makeShared<ModrinthCreationTask>(m_stagingPath, m_trustedSource, m_parent, packId, packVersionId, originalInstanceId);
    } else {
        QString packId;
        ...
        instCreationTask = makeShared<ModrinthCreationTask>(m_stagingPath, m_trustedSource, m_parent, packId);
    }
