...
concept ConfigObject = requires(T x) {
    { T::load(QString()) } -> std::same_as<Result<T>>;
    { x.save(QString()) } -> std::same_as<Result<>>;
   ...
   public:
    ...
    explicit ConfigHolder(QString path, const T& config) : m_path(std::move(path)), m_config(config)
    {
        ...
        connect(&m_saveTimer, &QTimer::timeout, this, [this] {
            if (const auto result = m_config.save(m_path); !result) {
                qWarning() << "Delayed config auto-save failed:" << result.error();
            }
        });
    }
    ...
    ~ConfigHolder() override
    {
        if (savePending()) {
            if (const auto result = m_config.save(m_path); !result) {
                qWarning() << "Terminal config auto-save failed:" << result.error();
            }
        }
    }
    ...
    {
        TRY_INTO(const auto& newConfig, T::load(m_path));
