...
GlobalConfig loadConfig(const INIFile& file)
{
    ...
    conf.backgroundCat = file.convert<QString>("BackgroundCat");
    ...
    conf.consoleMaxLines = file.convert("ConsoleMaxLines", 100'000);
    conf.consoleMaxLines = std::max(conf.consoleMaxLines, 1);
    conf.consoleOverflowStop = file.convert("ConsoleOverflowStop", true);

    conf.instanceDir = file.convert<QString>("InstanceDir");
    ...
    conf.centralModsDir = file.convert<QString>("CentralModsDir");
    ...
    conf.proxyType = file.convert<QString>("ProxyType");
    ...
    conf.totalPlayTime = int64_t(file.convert<qlonglong>("TotalPlayTime"));
    ...
    conf.downloadGameFilesDuringInstanceCreation = file.convert("DownloadGameFilesDurationInstanceCreation", true);

    conf.lastOfflinePlayerName = file.convert("LastOfflinePlayerName", QString());
    ...
    conf.instRenamingMode = file.convert("InstRenamingMode", "AskEverytime");
    ...
    conf.pastebinType = file.convert("PastebinType", PasteUpload::Type::Mclogs);
    ...
    if (!pastebinURL.isEmpty() && pastebinURL != PasteUpload::Type(PasteUpload::Type::NullPointer).defaultBase()) {
        ...
        conf.pastebinCustomApiBase = file.convert<QString>("PastebinCustomAPIBase");
    }

    // FIXME: no longer resets URL
    conf.metaUrlOverride = file.convert<QString>("MetaURLOverride");
    conf.resourceUrlOverride = file.convert("ResourceURLOverride", file.convert<QString>("ResourceURL"));
    conf.legacyFmlLibsUrlOverride = file.convert<QString>("LegacyFMLLibsURLOverride");
