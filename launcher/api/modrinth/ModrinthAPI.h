...
class ModrinthAPI : public ProviderAPI {
   public:
    static void registerClass() { ProviderAPI::registerClass(std::unique_ptr<ModrinthAPI>(new ModrinthAPI())); }
    virtual ~ModrinthAPI() = default;
   ...
   protected:
    ...
    [[nodiscard]] virtual std::unique_ptr<HttpRequest> prepareGetFileChangelogRequest(VersionArgs) const { return nullptr; }
    ...
    [[nodiscard]] virtual std::unique_ptr<HttpRequest> prepareGetLatestVersionsRequest(GetLatestVersionsArgs const& args) const;
    ...
    [[nodiscard]] virtual bool handleGetLatestVersionsResponse(const QJsonDocument& doc, GetLatestVersionsResponse& rsp) const;
};
