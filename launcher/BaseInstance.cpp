    ...
    : m_rootDir(std::move(rootDir)), m_config(std::move(conf))
{
    if (config()->uuid.isEmpty()) {
        m_config->update().regenerateUuid();
    }
}
...
void BaseInstance::resetAutoJavaInstallation()
{
    if (APPLICATION->config()->automaticJavaSwitch && config()->automaticJava && config()->javaInstallation.has_value()) {
        config().update().javaInstallation = std::nullopt;
    }
}
