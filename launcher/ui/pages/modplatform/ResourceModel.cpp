...
auto ResourceModel::data(const QModelIndex& index, int role) const -> QVariant
{
    ...
    switch (role) {
        ...
        case Qt::DecorationRole: {
            if (APPLICATION_DYN) {
                if (auto iconOrNone = const_cast<ResourceModel*>(this)->getIcon(index, pack->logoUrl);
...
void ResourceModel::searchRequestSucceeded(QList<ModPlatform::IndexedPack::Ptr>& newList)
{
    ...
    beginInsertRows(QModelIndex(), m_packs.size(), static_cast<int>(m_packs.size() + filteredNewList.size() - 1));
