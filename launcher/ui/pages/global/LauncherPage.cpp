...
void LauncherPage::applySettings()
{
    ...
    switch (ui->modUpdateChannelComboBox->currentIndex()) {
        case 1:
            conf.modUpdateReleaseTypes = {"release"};
            break;
        case 2:
            conf.modUpdateReleaseTypes = { "release", "beta" };
            break;
        case 0:
        default:
            conf.modUpdateReleaseTypes = {};
            break;
    }
}
void LauncherPage::loadSettings()
{
    ...
    const auto& releaseTypes = conf.modUpdateReleaseTypes;
    if (releaseTypes.size() == 1 && releaseTypes.contains("release", Qt::CaseInsensitive)) {
        ui->modUpdateChannelComboBox->setCurrentIndex(1);
    } else if (releaseTypes.size() == 2 && releaseTypes.contains("release", Qt::CaseInsensitive) &&
               releaseTypes.contains("beta", Qt::CaseInsensitive)) {
