   ...
   public:
    ...
    virtual QVariant data(const QModelIndex& index, int role = Qt::DisplayRole) const
    {
        ...
        if (index.column() == 0 && role == Qt::DecorationRole) {
            ...
            auto iconFile = worlds->data(sourceIndex, MultiWorldList::IconFileRole).toString();
            if (iconFile.isNull()) {
...
void MultiWorldListPage::closedImpl()
{
...
MultiWorldListPage::~MultiWorldListPage()
{
    m_worlds->stopWatching();
    delete ui;
}

void MultiWorldListPage::ShowContextMenu(const QPoint& pos)
{
...
QMenu* MultiWorldListPage::createPopupMenu()
{
...
bool MultiWorldListPage::shouldDisplay() const
{
    return true;
}

void MultiWorldListPage::retranslate()
{
    ui->retranslateUi(this);
}

bool MultiWorldListPage::worldListFilter(QKeyEvent* keyEvent)
{
...
bool MultiWorldListPage::eventFilter(QObject* obj, QEvent* ev)
{
...
void MultiWorldListPage::on_actionRemove_triggered()
{
...
void MultiWorldListPage::on_actionData_Packs_triggered()
{
    ...
    const QString fullPath = m_worlds->data(index, MultiWorldList::FolderRole).toString();
    const QString folder = FS::PathCombine(fullPath, "datapacks");

    auto dialog = new QDialog(this);
    dialog->setWindowTitle(tr("Data packs for %1").arg(m_worlds->data(index, MultiWorldList::NameRole).toString()));
...
QModelIndex MultiWorldListPage::getSelectedWorld()
{
...
void MultiWorldListPage::on_actionCopy_Seed_triggered()
{
    QModelIndex index = getSelectedWorld();
    ...
    int64_t seed = m_worlds->data(index, MultiWorldList::SeedRole).toLongLong();
    APPLICATION->clipboard()->setText(QString::number(seed));
}

void MultiWorldListPage::on_actionMCEdit_triggered()
{
    if (m_mceditStarting)
        return;
    ...
    auto fullPath = m_worlds->data(index, MultiWorldList::FolderRole).toString();
    ...
    if (program.size()) {
        ...
        connect(m_mceditProcess.get(), &LoggedProcess::stateChanged, this, &MultiWorldListPage::mceditState);
...
void MultiWorldListPage::mceditError()
{
    QMessageBox::warning(this->parentWidget(), tr("MCEdit failed to start!"),
                         tr("MCEdit failed to start.\nIt may be necessary to reinstall it."));
}

void MultiWorldListPage::mceditState(LoggedProcess::State state)
{
    bool failed = false;
    switch (state) {
...
void MultiWorldListPage::worldChanged([[maybe_unused]] const QModelIndex& current, [[maybe_unused]] const QModelIndex& previous)
{
    QModelIndex index = getSelectedWorld();
...
void MultiWorldListPage::on_actionCopy_triggered()
{
    QModelIndex index = getSelectedWorld();
    if (!index.isValid()) {
        return;
