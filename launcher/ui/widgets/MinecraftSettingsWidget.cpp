...
void MinecraftSettingsWidget::loadSettings()
{
    ...
    if (m_instance != nullptr) {
        ...
        if (overrideLoaders) {
            ...
            m_ui->neoForge->setChecked((instLoaders & ModPlatform::NeoForge) != 0);
            m_ui->forge->setChecked((instLoaders & ModPlatform::Forge) != 0);
            m_ui->fabric->setChecked((instLoaders & ModPlatform::Fabric) != 0);
            m_ui->quilt->setChecked((instLoaders & ModPlatform::Quilt) != 0);
            m_ui->liteLoader->setChecked((instLoaders & ModPlatform::LiteLoader) != 0);
            m_ui->babric->setChecked((instLoaders & ModPlatform::Babric) != 0);
            m_ui->btaBabric->setChecked((instLoaders & ModPlatform::BTA) != 0);
            m_ui->legacyFabric->setChecked((instLoaders & ModPlatform::LegacyFabric) != 0);
            m_ui->ornithe->setChecked((instLoaders & ModPlatform::Ornithe) != 0);
            m_ui->rift->setChecked((instLoaders & ModPlatform::Rift) != 0);
        }
