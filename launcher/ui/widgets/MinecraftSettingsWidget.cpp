    ...
    : QWidget(parent), m_instance(instance), m_ui(new Ui::MinecraftSettingsWidget)
{
    ...
    if (m_instance == nullptr) {
        ...
        connect(&m_instance->config(), &InstanceConfigHolder::updated, this,
                [this]() { m_ui->latestMCVersionGroupBox->setChecked(m_instance->config()->useLatestMinecraftVersionType.has_value()); });
    }
...
void MinecraftSettingsWidget::loadSettings()
{
    if (m_instance != nullptr) {
        ...
        const auto& latestVersionType = conf.useLatestMinecraftVersionType;
        m_ui->latestMCVersionGroupBox->setChecked(latestVersionType.has_value());
        if (latestVersionType.has_value()) {
            auto autoUpdateType = latestVersionType.value() == "release";
            m_ui->releaseRadioButton->setChecked(autoUpdateType);
            m_ui->anyRadioButton->setChecked(!autoUpdateType);
        } else {
            m_ui->releaseRadioButton->setChecked(true);
        }
...
void MinecraftSettingsWidget::loadSettingsFrom(const T& conf, const GlobalConfig& global)
{
    ...
    m_ui->customCommands->setCommands(commands.preLoad, commands.preLaunch, commands.wrapper, commands.postExit);
...
void MinecraftSettingsWidget::saveSettings()
{
    if (m_instance != nullptr) {
        ...
        if (m_ui->latestMCVersionGroupBox->isChecked()) {
            conf.useLatestMinecraftVersionType = QString(m_ui->releaseRadioButton->isChecked() ? "release" : "any");
        }
    } else {
...
void MinecraftSettingsWidget::saveSettingsTo(T& conf) const
{
    ...
    if (m_ui->customCommands->checked()) {
        conf.commands = GlobalConfig::CommandOverrides{
            .preLoad = m_ui->customCommands->preLoadCommand(),
...
void MinecraftSettingsWidget::updateAccountsMenu(const InstanceConfig& conf) const
{
