...
int MSALoginDialog::exec()
{
    ...
    connect(m_authflowTask.get(), &Task::failed, this, &MSALoginDialog::onAuthFlowTaskFailed);
    ...
    connect(m_devicecodeTask.get(), &Task::failed, this, &MSALoginDialog::onDeviceCodeTaskFailed);
...
void MSALoginDialog::onAuthFlowTaskFailed(const QString& reason)
{
    // Set message
    m_authflowTask->disconnect();
    m_ui->stackedWidget2->setCurrentIndex(0);
    m_ui->status2->setText(formatError(reason));
    m_ui->loadingLabel2->setText(m_authflowTask->getStatus());
    disconnect(m_ui->buttonBox->button(QDialogButtonBox::Cancel), &QPushButton::clicked, m_authflowTask.get(), &Task::abort);
    if (m_devicecodeTask->getState() == Task::State::Failed) {
...
void MSALoginDialog::onDeviceCodeTaskFailed(const QString& reason)
{
    // Set message
    m_devicecodeTask->disconnect();
    m_ui->stackedWidget->setCurrentIndex(0);
    m_ui->status->setText(formatError(reason));
    m_ui->loadingLabel->setText(m_devicecodeTask->getStatus());
    disconnect(m_ui->buttonBox->button(QDialogButtonBox::Cancel), &QPushButton::clicked, m_devicecodeTask.get(), &Task::abort);
    if (m_authflowTask->getState() == Task::State::Failed) {
        disconnect(m_ui->buttonBox->button(QDialogButtonBox::Cancel), &QPushButton::clicked, m_authflowTask.get(), &Task::abort);
        connect(m_ui->buttonBox->button(QDialogButtonBox::Cancel), &QPushButton::clicked, this, &MSALoginDialog::reject,
                Qt::UniqueConnection);
    }
