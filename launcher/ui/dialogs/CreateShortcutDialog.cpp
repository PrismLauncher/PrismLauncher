    ...
    : QDialog(parent), ui(new Ui::CreateShortcutDialog), m_instance(instance)
{
    ...
    if (m_QuickJoinSupported) {
        for (const auto& instanceWorld : worldList->allWorlds()) {
            // Entry name: World Name [Game Mode] - Last Played: DateTime
            QString entry_name = tr("%1 [%2] - Last Played: %3")
                                     .arg(instanceWorld.world.name(), instanceWorld.world.gameType().toTranslatedString(), instanceWorld.world.lastPlayed().toString(Qt::ISODate));
            ui->worldSelectionBox->addItem(entry_name, instanceWorld.world.name());
        }
    }
