...
class MetaEntry {
   ...
   public:
    ...
    void setRemoteChangedTimestamp(QString remote_changed_timestamp) { m_remote_changed_timestamp = std::move(remote_changed_timestamp); }
...
class HttpMetaCache : public QObject {
   ...
   public:
    ...
    auto getEntry(const QString& base, const QString& resource_path) -> MetaEntryPtr;

    // get the entry from cache and verify that it isn't stale (within reason)
    auto resolveEntry(const QString& base, QString resource_path, const QString& expected_etag = QString()) -> MetaEntryPtr;

    // add a previously resolved stale entry
    auto updateEntry(const MetaEntryPtr& stale_entry) -> bool;

    // evict selected entry from cache
    auto evictEntry(const MetaEntryPtr& entry) -> bool;
    bool evictAll();

    void addBase(const QString& base, QString base_root);
   ...
   private:
    ...
    auto staleEntry(const QString& base, QString resource_path) -> MetaEntryPtr;
