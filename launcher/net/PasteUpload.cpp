...
auto PasteUpload::make(const QString& log, QString baseUrl, PasteType pasteType) -> std::pair<Net::NetRequest::Ptr, QString*>
{
    ...
    Net::NetRequest::PostData postData{ nullptr };
    switch (pasteType) {
    ...
    auto parseFunc = [baseUrl, url, pasteType](const QByteArray& response) -> Net::RpcSink<QString>::ParseResult {
        switch (pasteType) {
    ...
    auto dl = Net::NetRequest::makeCustomRequest(url, std::move(postData), Net::NetRequest::HttpMethod::Post);
    if (multiPart) {
