...
Task::State MetaCacheSink::finalizeCache(QNetworkReply & reply)
{
    ...
    { // Cache lifetime
        if (m_is_eternal) {
            qCDebug(LAUNCHER_LOG) << "[MetaCache] Adding eternal cache entry:" << m_entry->getFullPath();
            m_entry->makeEternal(true);
        } else if (reply.hasRawHeader("Cache-Control")) {
            auto cache_control_header = reply.rawHeader("Cache-Control");
            // qCDebug(LAUNCHER_LOG) << "[MetaCache] Parsing 'Cache-Control' header with" << cache_control_header;
            ...
            // qCDebug(LAUNCHER_LOG) << "[MetaCache] Parsing 'Expires' header with" << expires_header;
        ...
        if (reply.hasRawHeader("Age")) {
            ...
            // qCDebug(LAUNCHER_LOG) << "[MetaCache] Parsing 'Age' header with" << age_header;
