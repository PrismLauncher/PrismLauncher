...
void InstanceCopyTask::copyFinished()
{
    ...
    if (!conf.has_value()) {
        emitFailed(tr("Could not read copied instance config: %1").arg(conf.error()));
        return;
    }
    ...
    if (m_useLinks) {
        ...
        auto allowedSymlinksFile = QFileInfo(FS::PathCombine(m_stagingPath, "allowed_symlinks.txt"));
