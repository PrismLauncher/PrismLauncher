...
void LaunchController::launchInstance()
{
    ...
    // NOTE: don't reload if a save is pending as we will lose changes!
    if (!m_instance->config().savePending() && !m_instance->config().reload()) {
        if (const auto reloadResult = m_instance->config().reload(); !reloadResult) {
            const auto message = tr("Instance reload failed: %1").arg(reloadResult.error());
...
void LaunchController::readyForLaunch()
{
    ...
    connect(profilerInstance, &BaseProfiler::readyToLaunch, this, [this](const QString& message) {
        ...
        if (msg.checkBox()->isChecked()) {
            m_launcher->instance()->config().update().profiler = "";
        }
