...
class BasicTask : public TaskV2 {
    Q_OBJECT

    friend class TaskTest;

   public:
    BasicTask(const char* categoryName = "launcher.task", QtMsgType enableForLevel = QtDebugMsg)
        : TaskV2(nullptr, categoryName, enableForLevel)
    {}
...
/* Does nothing. Only used for testing. */
class BasicTask_MultiStep : public TaskV2 {
    Q_OBJECT
...
class BigConcurrentTaskThread : public QThread {
    ...
    void run() override
    {
        ...
        connect(&big_task, &TaskV2::finished, this, &QThread::quit);
...
class TaskTest : public QObject {
   ...
   private slots:
    ...
    void test_SetProgress()
    {
        ...
        t.setProgressTotal(total);
        t.setProgress(current);

        QCOMPARE(t.progress(), current);
        QCOMPARE(t.progressTotal(), total);
    }

    void test_basicRun()
    {
        BasicTask t;
        QObject::connect(&t, &TaskV2::finished,
    ...
    void test_basicConcurrentRun()
    {
        ...
        QObject::connect(&t, &TaskV2::finished, [&t, &t1, &t2, &t3] {
            QVERIFY2(t.wasSuccessful(), "Task finished but was not successful when it should have been.");
    ...
    void test_moreConcurrentRun()
    {
        ...
        QObject::connect(&t, &TaskV2::finished, [&t, &t1, &t2, &t3, &t4, &t5, &t6, &t7, &t8, &t9] {
            QVERIFY2(t.wasSuccessful(), "Task finished but was not successful when it should have been.");
            QVERIFY(t1->wasSuccessful());
            QVERIFY(t2->wasSuccessful());
            QVERIFY(t3->wasSuccessful());
